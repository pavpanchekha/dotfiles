...
| eat             | Handle curses commands in eshell         |
...
Let's move toward obsoleting the ASCII TAB character.
...
When I teach or project it's good to have light mode, but I prefer to code in dark mode. "<leader>t" toggles between them
...
#+BEGIN_SRC emacs-lisp
  ...
  (evil-leader/set-key "t" #'toggle-tango-themes)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
    ...
    (evil-leader/set-key "l" #'my/latexmk-start)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defvar pavel/emacs-frontmost-p t)

  (add-hook 'focus-in-hook  (lambda () (setq pavel/emacs-frontmost-p t)))
  (add-hook 'focus-out-hook (lambda () (setq pavel/emacs-frontmost-p nil)))
  ...
  (defun pavel/eshell-maybe-notify ()
    (unless pavel/emacs-frontmost-p
...
#+BEGIN_SRC emacs-lisp
    ...
    even if you abort."
      ...
      (let* ((orig-buf    (current-buffer))
             ...
             (dir         (read-directory-name "Directory for HTTP server: ")))
        (unwind-protect
            (progn
              ...
              (with-current-buffer web-server--buffer-name
                (read-only-mode 1)
                (goto-char (point-max))))
          ;; always restore original window & buffer
          (when (window-live-p orig-win)
            (select-window orig-win)
            (switch-to-buffer orig-buf))))

      (evil-leader/set-key "v" #'start-python-http-server)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (evil-leader/set-key "a" #'amp-magit-start-thread)
#+END_SRC
