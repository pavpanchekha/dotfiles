...
* Packages
  ...
  + coffee-mode :: nice syntax for Javascript.
  + multi-term  :: Emacs =term= is impossible.  This is not.
  + jabber      :: Google chat in my Emacs.
  + magit       :: =git= in Emacs
...
I also need to periodically sync MobileOrg.  Here's how I accomplish
that, with code from [[http://stackoverflow.com/questions/8432108/how-to-automatically-do-org-mobile-push-org-mobile-pull-in-emacs][StackOverflow]].
...
I've started using NotMuch for my email needs.  It is much faster than
GNUS, and also has the distinct positive of working well with
IMAP-synced Maildir.
...
My mail gets sent through the GMail IMAP server using =msmtp=.  The
configuration for that isn't in the =dotfiles= repository since it
contains passwords.
...
Finally, I have a bunch of tags I might want to apply to mail.  As
much as possible, the key bindings here mirror the GMail key bindings.

#+BEGIN_SRC emacs-lisp
  (defmacro define-notmuch-key (key how tag)
    `(define-key notmuch-search-mode-map (kbd ,key)
       (lambda () (interactive)
         ,(cond
           ((eq how :toggle)
            `(notmuch-search-tag
              (concat (if (member ,tag (notmuch-search-get-tags))
                          "-" "+")
                      ,tag)))
           ((eq how :set)
  ...
  (eval-after-load 'notmuch-search
    (progn
       ...
       (define-key notmuch-search-mode-map (kbd "X")
         '(lambda () (interactive)
            (let ((changes (notmuch-read-tag-changes "" "")))
              (apply 'notmuch-tag "tag:marked" changes))
            (notmuch-search-refresh-view)))

       (define-key notmuch-search-mode-map (kbd "M-x")
         '(lambda () (interactive)
            (apply 'notmuch-tag "tag:marked" '("-marked"))
            (notmuch-search-refresh-view)))
  
       (define-notmuch-key "!" :toggle "spam")
       (define-notmuch-key "#" :toggle "deleted")
       (define-notmuch-key "e" :toggle "inbox")
       (define-notmuch-key "I" :toggle "unread")
       (define-notmuch-key "i" :toggle "keep")
       (define-notmuch-key "x" :toggle "marked")))
#+END_SRC

I also would like my messages to be PGP-signed by default.

#+BEGIN_SRC emacs-lisp
(add-hook 'message-setup-hook 'mml-secure-sign-pgpmime)
#+END_SRC

The default search order is wonky.  And, GMail creates ugly HTML
email.  Since Emacs isn't rich-text capable anyway, why not hide the
HTML parts by default?
