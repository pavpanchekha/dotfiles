...
I usually like to build LaTeX with =latexmk=, and this command starts
=latexmk= in preview mode in the current directory---very handy.
Basically, it first identifies the main file, among =main.tex=,
=paper.tex=, and =letter.tex= (for recommendation letters):
...
I keep an (org-mode) journal for random notes (not for journaling)
