...
We also want to explain to Gnus where to fine mail.  This is the
inverse operation.  We're going to tell Gnus to use the =ml= format;
one file per message seems cleanest.
