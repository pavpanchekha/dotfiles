...
I use Emacs's package manager to install others' Emacs scripts and helpers. I just switched from Marmalade to MELPA for packages, so I need to add MELPA.
...
Evil also binds =SPC= and =RET=, which have pretty useless bindings and are often used in other modes. This removes that binding:
...
I use =dired= a lot, but by default it’s hard to open a directory in =ido= mode. This setting adds =./= to the =ido= completion list.
...
#+BEGIN_SRC emacs-lisp
  (setf org-journal-dir "~/Dropbox/Journal/")
  (setf org-journal-file-format "%Y-%m-%d.txt")
...
But Markdown is standard in the open-source community, so I load support.
...
I used to use [[https://github.com/pavpanchekha/center-text.el][center-text mode]], which increases the fringe to keep the content narrow, but to be honest that was quite a pain in a lot of ways. I should improve that mode, but until now I’m keeping =center-text-mode= off.

#+BEGIN_SRC emacs-lisp :tangle no
  (package-require 'center-text-mode)
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun dired-hide-details-home ()
    ...
    (when (and buffer-file-name
               (equal buffer-file-name (expand-file-name "~")))
...
Multiple methods of indenting Haskell code come with Haskell Mode. They don't differ much, but I prefer =haskell-indentation=. I used to use =haskell-indent= but it annoyed me somehow.
