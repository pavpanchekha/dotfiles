...
#+BEGIN_SRC emacs-lisp
  (defmacro define-notmuch-key (key how tag)
    `(define-key notmuch-search-mode-map (kbd ,key)
       (lambda () (interactive)
         ,(cond
           ((eq how :toggle)
            `(notmuch-search-tag-thread
              (concat (if (member ,tag (notmuch-search-get-tags))
                          "-" "+")
                      ,tag)))
           ((eq how :set)
            `(notmuch-search-tag-thread (concat "+" ,tag)))
           ((eq how :unset)
            `(notmuch-search-tag-thread (concat "-" ,tag)))))))
  ...
  (eval-after-load 'notmuch-search
    '(progn
       ...
       (define-notmuch-key "S-1" :toggle "spam")
       (define-notmuch-key "S-3" :toggle "deleted")
       (define-notmuch-key "e"   :toggle "inbox")
       (define-notmuch-key "S-i" :toggle "unread")
...
The default search order is wonky.
