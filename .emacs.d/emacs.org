...
My mail gets sent through the GMail IMAP server.  I used to use =msmtp=
for sending mail, but I've now switched to using =smtpmail.el=, since
it's nice to have an Emacs-native solution.
