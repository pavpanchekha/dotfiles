...
Emacs's default =yes or no= prompts annoy. I ask Emacs to accept =y= and =n=.
...
I also use Markdown and BibTeX for some files. Markdown has become a standard format for many in the open-source community. My BibTeX files contain commentaries and so are also text files.

#+BEGIN_SRC emacs-lisp
  (package-safe-install 'markdown)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (dolist (hook '(bibtex-mode-hook markdown-mode-hook org-mode-hook))
    (add-hook hook 'variable-pitch-mode)
    (add-hook hook 'flyspell-mode))
#+END_SRC

The visual-line mode implements proper line wrapping. I prefer to use this, but traditionally BibTeX and Markdown files do not use it. If a window's width is very large, visual lines can become nigh-unreadable. My [[https://github.com/pavpanchekha/center-text.el][center-text mode]] narrows the width of buffer contents to account for this.

#+BEGIN_SRC emacs-lisp
  (add-to-list 'load-path "~/Dropbox/Work/center-text.el/")
...
Gesier stores history information; I'd prefer it not clutter my home directory.

#+BEGIN_SRC emacs-lisp
  (setf geiser-repl-history-filename "~/.emacs.d/gesier-history")
#+END_SRC
